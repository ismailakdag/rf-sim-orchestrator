...
def _deadline(job: dict[str, Any]) -> datetime:
    ...
    if raw is None and "deadline_utc" in job:
        return None
    if not raw:
...
def run(job_path: Path) -> int:
    ...
    try:
        ...
        record["deadline_utc"] = deadline.isoformat() if deadline is not None else None
...
End With'''))
        ...
        while project.model3d.is_solver_running(timeout=STATUS_CALL_TIMEOUT_SECONDS):
            ...
            if job.get("timeout_seconds", 1800) is not None and elapsed > float(job.get("timeout_seconds", 1800)):
